...
load_data <- function(N) {
     ...
     train_input <- as.list(enumerate(isplitCols(data$trainData[1:D, 1:(N*M)], chunks = M)))
     train_target <- as.list(enumerate(isplitCols(data$trainData[D + 1, 1:(N*M), drop=F], chunks = M)))
